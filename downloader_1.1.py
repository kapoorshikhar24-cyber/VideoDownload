...
class DownloaderGUI:
    ...
    def build_single_tab_ui(self):
        ...
        url_lbl = ttk.Label(url_card, text="Media / Video URL", style='Title.Card.TLabel')
        ...
        placeholder_lbl = ttk.Label(
            ...
            text="Paste a media URL and click 'Analyze Link'\nto reveal video formats and download options.",
            font=('Segoe UI', 11),
            foreground=self.colors['text_muted'],
            justify='center',
            style='Card.TLabel'
        ...
        self.ext_lbl = ttk.Label(format_row, text="Ext:", style='Card.TLabel', font=('Segoe UI', 10, 'bold'))
        self.ext_lbl.pack(side='left', padx=(10, 5))
        
        self.ext_var = tk.StringVar(value="mp4")
        self.ext_combobox = ttk.Combobox(
            format_row, textvariable=self.ext_var, state='readonly', width=6, style='TCombobox'
        )
        self.ext_combobox['values'] = ("mp4", "mkv", "webm", "avi", "flv", "mov")
        self.ext_combobox.pack(side='left')
    ...
    def on_format_type_changed(self):
        if self.format_type.get() == "audio":
            ...
            self.ext_combobox['values'] = ("mp3", "m4a", "wav", "flac")
    ...
    def download_worker(self, url, save_path):
        ...
        if audio_only:
            ydl_opts.update({
                ...
                'postprocessors': [{
                    'key': 'FFmpegExtractAudio',
                    'preferredcodec': getattr(self, 'ext_var', tk.StringVar(value='mp3')).get(),
                    'preferredquality': '192' if 'Medium' in quality else ('320' if 'Best' in quality else '128'),
                }],
            })
        else:
            target_ext = getattr(self, 'ext_var', tk.StringVar(value='mp4')).get()
            ydl_opts.update({'merge_output_format': target_ext})
    ...
    def build_playlist_tab_ui(self):
        ...
        p_url_lbl = ttk.Label(p_url_card, text="Playlist URL (YouTube, SoundCloud, etc.)", style='Title.Card.TLabel')
        ...
        self.p_ext_lbl = ttk.Label(p_format_row, text="Ext:", style='Card.TLabel', font=('Segoe UI', 10, 'bold'))
        self.p_ext_lbl.pack(side='left', padx=(10, 5))
        
        self.p_ext_var = tk.StringVar(value="mp4")
        self.p_ext_combobox = ttk.Combobox(
            p_format_row, textvariable=self.p_ext_var, state='readonly', width=6, style='TCombobox'
        )
        self.p_ext_combobox['values'] = ("mp4", "mkv", "webm", "avi", "flv", "mov")
        self.p_ext_combobox.pack(side='left')
    ...
    def on_playlist_format_type_changed(self):
        if self.p_format_type.get() == "audio":
            ...
            self.p_ext_combobox['values'] = ("mp3", "m4a", "wav", "flac")
            self.p_ext_var.set("mp3")
        else:
            self.p_quality_lbl.configure(text="Quality / Resolution:")
            self.p_quality_combobox['values'] = ("Best Quality", "1080p", "720p", "480p", "360p")
            self.p_quality_var.set("Best Quality")
            self.p_ext_combobox['values'] = ("mp4", "mkv", "webm", "avi", "flv", "mov")
            self.p_ext_var.set("mp4")
        ...
        for index, entry in enumerate(entries, 1):
            ...
            if audio_only:
                ydl_opts.update({
                    ...
                    'postprocessors': [{
                        'key': 'FFmpegExtractAudio',
                        'preferredcodec': getattr(self, 'p_ext_var', tk.StringVar(value='mp3')).get(),
                        'preferredquality': '192' if 'Medium' in quality else ('320' if 'Best' in quality else '128'),
                    }],
                })
            else:
                target_ext = getattr(self, 'p_ext_var', tk.StringVar(value='mp4')).get()
                ydl_opts.update({'merge_output_format': target_ext})
